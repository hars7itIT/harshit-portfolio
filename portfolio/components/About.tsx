    ...
    <section id="about" className="mx-auto max-w-5xl px-6 py-24 relative">
      ...
      <div className="mt-12 grid gap-12 lg:grid-cols-5">
        ...
        <motion.div 
          ...
          className="lg:col-span-2 flex flex-col justify-between rounded-2xl border border-white/5 bg-slate-950/40 p-6 shadow-xl backdrop-blur-md"
        >
          <div>
          ...
          <div className="mt-8 border-t border-white/5 pt-6 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-white text-xs">Continuously Learning & Building</div>
            </div>
          </div>
        </motion.div>

        {/* Right Card: Interactive Timeline of Traits */}
        <div className="lg:col-span-3 relative border-l border-white/10 pl-8 space-y-8">
          {TRAITS.map((t, i) => {
            const Icon = t.icon;
            return (
              <motion.div
              ...
              >
                ...
                <span className="absolute -left-[45px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-white/10 bg-slate-950 flex items-center justify-center transition-all group-hover:border-cyan-400/80 group-hover:shadow-[0_0_8px_rgba(6,182,212,0.8)]">
                  <span className="h-1.5 w-1.5 rounded-full bg-slate-800 transition-colors group-hover:bg-cyan-400" />
                </span>
                
                {/* Hover glassmorphic panel */}
                <div className="rounded-xl border border-transparent p-4 transition-all group-hover:border-white/5 group-hover:bg-white/[0.02] group-hover:backdrop-blur-sm">
                  <div className="flex items-center gap-3">
                    <Icon className={`h-5 w-5 ${t.color}`} />
                    <h3 className="font-mono text-sm font-semibold text-white">{t.title}</h3>
                  </div>
                  <p className="mt-2 text-sm text-slate-400 leading-relaxed">{t.detail}</p>
                </div>
              </motion.div>
            );
          })}
