        ...
        {entries.map((e, i) => {
          ...
          return (
            <div key={e.title} className="relative flex flex-col md:flex-row md:justify-between">
              ...
              {/* Timeline dot node */}
              <div className="absolute left-4 top-6 z-10 -ml-1.5 h-3 w-3 rounded-full border border-cyan-400 bg-ink shadow-[0_0_8px_rgba(6,182,212,0.8)] md:left-1/2 md:top-6 md:-translate-x-1/2 md:ml-0" />
              ...
              <div className={`w-full pl-10 md:w-[45%] md:pl-0 ${isEven ? "md:text-right md:order-1" : "md:order-3 md:invisible md:h-0"}`}>
                {isEven && (
                  ...
                  >
                    ...
                    <h3 className="mt-3 text-lg font-bold text-text leading-snug">{e.title}</h3>
                    <p className="mt-1 font-mono text-xs text-purple-400">{e.place}</p>
                    <p className="mt-3 text-sm text-muted leading-relaxed">{e.detail}</p>
                  </motion.div>
                )}
              </div>
